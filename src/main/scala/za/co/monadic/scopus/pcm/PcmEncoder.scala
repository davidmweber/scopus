...
case class PcmEncoder(sampleFreq: SampleFrequency, channels: Int) extends Encoder{
   ...
   */
  override def apply(audio: Array[Short]): Try[Array[Byte]] = Try(shortArrayToByteArray(audio))
   ...
   * @return An array containing the compressed audio or the exception in case of a failure
   */
  override def apply(audio: Array[Float]): Try[Array[Byte]] = Try(floatArrayToByteArray(audio))
